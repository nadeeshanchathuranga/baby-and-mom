  ...
  <div class="flex flex-wrap items-center gap-6">
    ...
    <div class="relative inline-block">
      <button
        ...
        class="md:px-12 py-4 px-4 md:text-lg font-bold tracking-wider text-white uppercase rounded-xl bg-amber-600 hover:bg-amber-700 cursor-pointer"
      >
        Expire Products
      </button>
      <span
    ...
    <div class="relative inline-block">
      <button
        ...
        :class="[
          'md:px-12 py-4 px-4 md:text-lg font-bold tracking-wider text-white uppercase rounded-xl cursor-pointer',
    ...
    <Link
      href="/add_promotion"
      :class="HasRole(['Admin'])
        ? 'px-12 py-4 text-lg font-bold tracking-wider text-white uppercase bg-cyan-600 rounded-xl'
        : 'px-12 py-4 text-lg font-bold tracking-wider text-white uppercase bg-cyan-600 cursor-not-allowed rounded-xl'"
    ...
    <p
      ...
      :class="HasRole(['Admin'])
        ? 'md:px-12 py-4 px-4 md:text-lg font-bold tracking-wider text-white uppercase bg-blue-600 rounded-xl'
        : 'md:px-12 py-4 px-4 md:text-lg font-bold tracking-wider text-white uppercase bg-blue-600 cursor-not-allowed rounded-xl'"
      :title="HasRole(['Admin']) ? '' : 'You do not have permission to add more Products'"
      class="select-none"
    >
      <i class="md:pr-4 ri-add-circle-fill"></i> Add More Product
            ...
            <div class="flex items-center space-x-4">
               ...
               <div class="md:w-1/4 w-full">
                  <input v-model="search" @input="performSearch" type="text" placeholder="Search ..."
            ...
            <div class="flex items-center space-x-4">
               ...
               <div class="flex justify-end w-full space-x-2">
                  ...
                  <!-- Color Filter -->
                  <select v-model="color" @change="applyFilters"
                     class="px-6 py-3 text-xl font-normal tracking-wider text-blue-600 bg-white rounded-lg custom-select">
                     <option value="">Select Color</option>
                     <option v-for="colorOption in props.colors" :key="colorOption.id" :value="colorOption.name">
                        {{ colorOption.name }}
                     </option>
                  </select>
                  <!-- Size Filter -->
                  <select v-model="size" @change="applyFilters"
                     class="px-6 py-3 text-xl font-normal tracking-wider text-blue-600 bg-white rounded-lg custom-select">
                     <option value="">Select Size</option>
                     <option v-for="sizeOption in props.sizes" :key="sizeOption.id" :value="sizeOption.name">
                        {{ sizeOption.name }}
                     </option>
                  </select>
                  <Link href="/products"
                  ...
                  <div v-for="product in products.data" :key="product.id"
                     ...
                     <div @click="
                        ...
                        <img :src="product.image
                           ...
                           " alt="Product Image" class="object-cover w-full h-64" />
                     </div>
                     <div class="px-2 py-4 space-y-4">
                        ...
                        <div class="flex items-center justify-between">
                           <p v-if="product.stock_quantity > 0"
                           ...
                           <div class="flex space-x-4">
                              <button :disabled="!HasRole(['Admin'])" @click="
                                 () => {
                                 if (HasRole(['Admin'])) {
            ...
            <div class="flex space-x-2 pagination">
               <!-- Prev Button -->
               <span v-if="products.links[0]" @click.prevent="navigateTo(products.links[0].url)" :class="[
                  'pagination-btn',
               ...
               <!-- Pagination Links -->
               <span v-for="(link) in products.links.slice(
                  1,
                  products.links.length - 1
                  )" :key="link.label" @click.prevent="navigateTo(link.url)"
                  :class="['pagination-btn', { 'pagination-active': link.active }]" v-html="link.label"></span>
               <!-- Next Button -->
               <span v-if="products.links[products.links.length - 1]" @click.prevent="
      ...
      <ProductCreateModel :categories="allcategories" :colors="colors" :sizes="sizes" :suppliers="suppliers"
         v-model:open="isCreateModalOpen" />
      <ProductUpdateModel :categories="allcategories" :colors="colors" :suppliers="suppliers" :sizes="sizes"
         v-model:open="isEditModalOpen" :selected-product="selectedProduct" />
      <ProductDuplicateModel :categories="allcategories" :colors="colors" :suppliers="suppliers" :sizes="sizes"
         v-model:open="isDuplicateModalOpen" :selected-product="selectedProduct" />
      <ProductViewModel :categories="allcategories" :colors="colors" :sizes="sizes" v-model:open="isViewModalOpen"
         :selected-product="selectedProduct" />
      <ProductDeleteModel v-model:open="isDeleteModalOpen" :selected-product="selectedProduct" @delete="deleteProduct" />
      <PreOrderProductModal :preOrderProducts="preOrderProducts" v-model:open="isPreOrderModalOpen" />
      <ExpireProductModal :expiryProducts="expiryProducts" v-model:open="isExpiryModalOpen" />
      <BulkBarcodeModal v-model="isBulkBarcodeModalOpen" :products="props.rawProducts" />
      <Footer />
</template>
<script setup>
